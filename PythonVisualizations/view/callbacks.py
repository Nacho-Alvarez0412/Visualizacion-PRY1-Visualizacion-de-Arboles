...
from visualizations.charts import create_icicle_chart, create_sunburst_chart


def register_callbacks(app, exportations_data):
    @app.callback(
        Output("sunburst-hs2-dropdown", "options"),
        Output("sunburst-chart", "figure"),
        Input("sunburst-section-dropdown", "value"),
        Input("sunburst-hs2-dropdown", "value")
    )
    def filter_sunburst_products(selected_section, selected_hs2):
        if (not selected_section and not selected_hs2) or (not selected_section and selected_hs2):
            return [], create_sunburst_chart(exportations_data)
